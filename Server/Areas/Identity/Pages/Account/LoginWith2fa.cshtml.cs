...
namespace AutenticacionBlazor.Server.Areas.Identity.Pages.Account
{
    ...
    public class LoginWith2faModel : PageModel
    {
        ...
        {
            [Required]
            [StringLength(7, ErrorMessage = "El {0} debe ser de al menos {2} y un máximo {1} caracteres de longitud.", MinimumLength = 6)]
            [DataType(DataType.Text)]
            [Display(Name = "Código de autenticador")]
            public string TwoFactorCode { get; set; }

            [Display(Name = "Recuerda este equipo")]
        ...
        public async Task<IActionResult> OnPostAsync(bool rememberMe, string returnUrl = null)
        {
            ...
            if (result.Succeeded)
            {
                _logger.LogInformation("Usuario con ID '{UserId}' iniciado sesión con 2fa.", user.Id);
            ...
            {
                _logger.LogWarning("Se ingresó un código de autenticación no válido para el usuario con ID '{UserId}'.", user.Id);
